...
import allocateSupplies from '../../services/allocateSuplies';

export default function AllocateSupplies({ hospital, inventory, assignmentsReload, onClose}) {
  const modalRef = useRef();
  const [masks, setmasks] = useState(false);
  const [kn95, setkn95] = useState('');
  const [faceShield, setFaceShield] = useState('');
  const [assignmentError, setAssignmentError] = useState(false);
  const [asignmenteSuccessfuly, setAsignmenteSuccessfully] = useState(false);
  ...
  const sendAssignmentForm = async () => {
    const suppliesQuantity = { masks, kn95, faceShield };
    const hospitalId = hospital.hospital_id;
    const data = { suppliesQuantity, hospitalId }
   
    try {
      const resp = await allocateSupplies(data);
      if (resp.success) {
        setAssignmentError(false);
        setAsignmenteSuccessfully(true);
        assignmentsReload();
      } else {
        setAssignmentError(true);
      }
    } catch (error) {
      console.log("🚀 ~ sendAssignmentForm ~ error:", error)
      console.error('Error al intentar asignar suministros:', error);
    }
  };
          ...
          <form className="mt-8 mb-2 w-3/5 max-w-screen-lg" onSubmit={handleFormSubmit}>
            <div className="mb-1 flex flex-col gap-6">
              <Typography variant="h6" color="blue-gray" className="-mb-3 font-agora">
                Cubrebocas (disponibles: {inventory[0].available_quantity})
              </Typography>
              <Input
                type='number'
                value={masks}
              ...
              />
              <Typography variant="h6" color="blue-gray" className="-mb-3 font-agora">
                Mascarillas KN95 (disponibles: {inventory[1].available_quantity})
              </Typography>
              <Input
                type="number"
                value={kn95}
              ...
              />
              <Typography variant="h6" color="blue-gray" className="-mb-3 font-agora">
                Caretas (disponibles: {inventory[2].available_quantity})
              </Typography>
              <Input
                type="number"
                value={faceShield}
            ...
            {!asignmenteSuccessfuly ? (
              ...
              <Typography color="gray" className="mt-4 text-center font-normal font-agora">
                Suministros asinados correctamente
              </Typography>
            )}
          </form>
        </div>
